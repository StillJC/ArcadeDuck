...
static constexpr char DISC_IMAGE_FILTER[] = QT_TRANSLATE_NOOP("MainWindow", "ArcadeDuck MAME Sets (*.zip)");
...
ALWAYS_INLINE static QString getWindowTitle(const QString& game_title)
{
  ...
  {
#if defined(_DEBUG)
    return qApp->translate("MainWindow", "ArcadeDuck [Debug]");
#elif defined(_DEBUGFAST)
    return qApp->translate("MainWindow", "ArcadeDuck [DebugFast]");
#else
    return qApp->translate("MainWindow", "ArcadeDuck");
#endif
  }

#if defined(_DEBUG)
  return qApp->translate("MainWindow", "ArcadeDuck - %1 [Debug]").arg(game_title);
#elif defined(_DEBUGFAST)
  return qApp->translate("MainWindow", "ArcadeDuck - %1 [DebugFast]").arg(game_title);
#else
  return qApp->translate("MainWindow", "ArcadeDuck - %1").arg(game_title);
#endif
}
