...
{
  static constexpr auto extensions = make_array(".zip"); // Arcade/MAME zip/CHD loading only.
...
static bool ExtractKonamiGVEepromFromZip(const std::string& zip_path, std::string_view set_name)
{
  ...
  if (const std::optional<std::vector<u8>> existing = FileSystem::ReadBinaryFile(eeprom_path.c_str());
      existing.has_value() && existing->size() == 0x80)
  {
    return true;
  }
  ...
  if (!zf)
  {
    Log_ErrorPrintf("KonamiGV: failed to open MAME zip '%s' for EEPROM extraction", zip_path.c_str());
    return false;
  }
...
static bool ResolveKonamiGVMameZipPath(std::string* path)
{
  ...
  const char* extension = std::strrchr(path->c_str(), '.');
  if (!extension || StringUtil::Strcasecmp(extension, ".zip") != 0)
    return false;
...
bool Boot(const SystemBootParameters& params)
{
  ...
  if (!boot_filename.empty())
  {
    ...
    if (exe_boot || psf_boot)
    {
      if (s_region == ConsoleRegion::Auto)
      {
        const DiscRegion file_region =
          (exe_boot ? GetRegionForExe(boot_filename.c_str()) : GetRegionForPsf(boot_filename.c_str()));
    ...
    {
      Log_InfoPrintf("Loading CD image '%s'...", boot_filename.c_str());

      media = OpenCDImage(boot_filename.c_str(), &error, params.load_image_to_ram, ShouldCheckForImagePatches());
      if (!media)
      {
        g_host_interface->ReportFormattedError("Failed to load CD image '%s': %s", boot_filename.c_str(),
      ...
      if (s_region == ConsoleRegion::Auto)
      {
        ...
        if (disc_region != DiscRegion::Other)
        {
          ...
          Log_InfoPrintf("Auto-detected console %s region for '%s' (region %s)",
                         Settings::GetConsoleRegionName(s_region), boot_filename.c_str(),
  ...
  UpdateRunningGame(media ? media->GetFileName().c_str() : boot_filename.c_str(), media.get());
...
static std::string GetKonamiGVMameSetNameFromResolvedCHDPath(const char* path)
{
  ...
  const char* extension = std::strrchr(path, '.');
  if (!extension || StringUtil::Strcasecmp(extension, ".chd") != 0)
    return {};

  const std::string_view path_view(path);
  const size_t chd_separator = path_view.find_last_of("/\\");
  if (chd_separator == std::string_view::npos || chd_separator == 0)
    return {};

  const size_t set_separator = path_view.find_last_of("/\\", chd_separator - 1);
  if (set_separator == std::string_view::npos)
    return {};
